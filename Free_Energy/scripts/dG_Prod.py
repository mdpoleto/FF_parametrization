...
def Production(lamb,moleculetype):
    inprod = "in.SIM.mdp"
    inprod_h = open(inprod, 'w')
    content = """; Run control
    ...
    lincs-order              = 12
        """
    inprod_h.write(content)
    inprod_h.write(L)
    inprod_h.write(content2)
    inprod_h.write(mol)
    inprod_h.write(content3)
    inprod_h.close()

    if not os.path.exists(prefix + ".SIM.gro"):
	if not os.path.exists(prefix + ".SIM.cpt"):
	    	min1gromppcommand = "gmx_" + ver + " grompp -f in.SIM.mdp -c " + prefix + ".NPT.gro -p " + prefix + ".top -t " + prefix + ".NPT.cpt -o " + prefix + ".SIM.tpr -po " + prefix + ".SIM.mdout.mdp -maxwarn 1"
    		os.system(min1gromppcommand)

	    	min1command = "gmx_" + ver + " mdrun -deffnm " + prefix + ".SIM -dhdl dhdl-"+ str(lamb)+".xvg -nb gpu"
